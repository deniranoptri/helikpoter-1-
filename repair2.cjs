...
for (let id in replacements) {
    ...
    // Instead of complex regex, let's just parse the file to find the block for the ID.
    // We can do this easily because the file has JSON-like structure.
    
    // Replace jawabanBenar:
    // Some options might have quotes inside them in the original text, like "Viralkan!". 
    // We can't just use [^"']*. Let's use string manipulation.
    ...
    // Find jawabanBenar after idIdx
    const jbMatch = /"jawabanBenar":\s*"(.*?)"(,\s*"pengecoh")/s.exec(content.substring(idIdx));
    if (jbMatch) {
        const jbStart = idIdx + jbMatch.index;
        // Construct the new string piece
        // Ensure double quotes are escaped if any
